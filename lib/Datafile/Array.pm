...
sub _csv_parse {
    ...
    while ($pos < $len) {
        my $field = '';
        if (substr($line, $pos, 1) eq '"') { # Quoted field
            $pos++;  # skip opening quote
            my $start = $pos;
            while (1) {
                my $qpos = index($line, '"', $pos);
                if ($qpos == -1) { # Unclosed quote: treat rest as literal (lenient)
                ...
                $pos = $qpos + 1;
                if ($pos < $len && substr($line, $pos, 1) eq '"') { # Escaped quote: keep one and continue
                    $field .= '"';
                    $pos++;
                    $start = $pos;
                } else { # Real closing quote
                    last;
                }
            }
        } else { # Unquoted field
...
sub readarray {
    ...
    my $has_headers = $opts->{has_headers}  // 1;    # no headerlines to expect 
    my $prefix      = $opts->{prefix}       // 0;
    my $search      = $opts->{search};               # string, qr//, or array ref
    ...
    my @messages = ();
    push @messages, "# readarray: fields=@field_names\n", "# opts: " . join(", ", map { "$_=$opts->{$_}" } sort keys %$opts) . "\n" if $verbose;

    # === PRE-COMPILE ALL SEARCH PATTERNS ONCE (outside the loop) ===
        ...
        unless ($header_done) {
            if ($has_headers > 0) {
                if ($has_headers == 1 || ($prefix && substr($line,0,1) eq 'H')) {
                    unless (@field_names) {
                       @field_names = map { _trim($_, 1) } @fields[$start_idx .. $#fields];
                       push @messages, "- header fields: @field_names\n" if $verbose;
	            }
                ...
                croak "datafile::array::readarray: no field names provided and none found in file" unless @field_names;
                $header_done = 1;
            }
        }
        next if @fields < $start_idx + @field_names;

        # === FAST AND LOGIC CHECK USING PRE-COMPILED REGEXES ===
	if (@compiled_patterns) {
            my $all_matched = 1;
            for my $regex (@compiled_patterns) {
                $all_matched = 0 unless $line =~ $regex;
                last unless $all_matched;  # early exit if one fails
            }
        ...
        if (ref $data eq 'HASH') {
            my @key_parts = map { _trim($fields[$_], $trim) } (0 .. $key_fields - 1);
    ...
    push @messages, "- $record_count data records read from $filename\n" if $verbose;
    return ($record_count, \@messages);
}

# writearray - unchanged from original
sub writearray {
    my ($filename, $data, $pafields, $opts) = @_;
    $opts //= {};
    ...
    if (ref $data ne 'ARRAY' && ref $data ne 'HASH' && !@field_names) {
         if (-f $filename) {
             unlink($filename) or return (0, [ "WARNING: unable to delete file $filename\n" ]);
    ...
    my @messages = ();
    push @messages, "# writearray: fields=@field_names\n", "# opts: " . join(", ", map { "$_=$opts->{$_}" } sort keys %$opts) . "\n"
        if $verbose;
    ...
    if ($header && @field_names) {
        ...
        push @messages, "> " . $prefix_hdr . join($delim, @field_names) . "\n" if $verbose;
    }

    my $record_count = 0;
    my @records = ref($data) eq 'HASH' ? sort keys %$data : 0 .. $#$data;
    ...
    if ($backup && -f $filename) {
       rename $filename, $filename.'.bak'
	       or push @messages, "WARNING: backup to ${filename}.bak failed: $!";
    }
    rename $tmp, $filename or return (0, [ "ERROR: failed to rename '$tmp' to '$filename': $!" ]);
    chmod $prot, $filename;

    push @messages, "- renamed $tmp to $filename\n", "- $record_count data records written to $filename\n" if $verbose;
    return ($record_count, \@messages);
}
